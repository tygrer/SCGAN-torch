...
if opt.align_data > 0 then
  ...
  require 'data.aligned_data_loader'
   opt.DATA_ROOT = './datasets/dehaze/align'
...
for epoch = 1, opt.niter+opt.niter_decay do
    epoch_tm:reset()
    for counter_in_epoch = 1, math.min(data_loader:size()+data_loader2:size(), opt.ntrain), opt.batchSize do
        tm:reset()
        -- load a batch and run G on that batch
        local real_dataA, real_dataB, _, _,data_CSRA, data_CSRB= data_loader:GetNextBatch()
		local real_data2A, real_data2B, _, _,data_CSR2A, data_CSR2B= data_loader2:GetNextBatch()
		--print(type(data_CSRA))
        model:Forward({real_A=real_dataA, real_B=real_dataB,real_A2=real_data2A, real_B2=real_data2B, CSRA=data_CSRA, CSRB=data_CSRB,CSRA2=data_CSR2A, CSRB2=data_CSR2B}, opt)
        -- run forward pass
        opt.counter = counter
        -- run backward pass
