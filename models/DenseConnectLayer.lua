...
--------------------------------------------------------------------------------
function DenseConnectLayerStandard(nChannels, bottomneck,droprate,grownrate)
   ...
   if bottomneck then
      ...
      net:add(cudnn.SpatialConvolution(nChannels, 4 * grownrate, 1, 1, 1, 1, 0, 0))
      nChannels = 4 * grownrate
      if droprate> 0 then net:add(nn.Dropout(droprate)) end
      net:add(cudnn.SpatialBatchNormalization(nChannels))
      net:add(cudnn.ReLU(true))      
   end
   net:add(cudnn.SpatialConvolution(nChannels, grownrate, 3, 3, 1, 1, 1, 1))
   if droprate> 0 then net:add(nn.Dropout(droprate)) end
...
function DenseConnectLayerCustom:__init(nChannels,bottomneck,droprate,grownrate)
   parent.__init(self)
   self.train = true
   self.opt = opt
   ...
   if bottomneck then
      ...
      self.net2:add(cudnn.SpatialConvolution(nChannels, 4*grownrate, 1, 1, 1, 1, 0, 0))
      nChannels = 4 * opt.growthRate
      self.net2:add(cudnn.SpatialBatchNormalization(nChannels))
      self.net2:add(cudnn.ReLU(true))
   end
   self.net2:add(cudnn.SpatialConvolution(nChannels, grownrate, 3, 3, 1, 1, 1, 1))
