...
if opt.model == 'cycle_gan' then
  require 'models.cycle_gan_model_original'
  model  = CycleGANModel()
...
for n = 1, math.floor(opt.how_many) do
  ...
  local cur_dataA, cur_dataB, cur_pathsA, cur_pathsB ,data_CSRA, data_CSRB = data_loader:GetNextBatchtest()
  ...
  model:Forward({real_A=cur_dataA, real_B=cur_dataB,CSRA=data_CSRA, CSRB=data_CSRB}, opt)
