...
function AlignedDataLoader:LoadBatchForAllDatasetstest()
  local batch_data, path = self.data:getBatch()
  --print('batch_size', batch_data:size())
  local batchA = batch_data[{ {}, self.idx_B, {}, {} }]
  local batchB = batch_data[{ {}, self.idx_A, {}, {} }]
  --print('*******************************',batch_data:size())
  --gm = require 'graphicsmagick'
  --image = gm.Image('test.jpg') 
  --image:show() --显示图像
  return batchA, batchB, path, path
end
function AlignedDataLoader:LoadBatchForAllDatasetstrain()
  local batch_data, path = self.data:getBatch()
  --print('batch_size', batch_data:size())
  local batchA = batch_data[{ {}, self.idx_B, {}, {} }]
  local batchB = batch_data[{ {}, self.idx_A, {}, {} }]
